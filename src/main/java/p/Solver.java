package p;
import java.io.*;
class Solver { // https://rosettacode.org/wiki/Sudoku
    ...
    static void file(File file) throws IOException {
        ...
        Filter.iterate(buffer,line-> {
            if(line.length()>0&&!line.startsWith("Puzzle")) {
                ...
                Solver solver=new Solver(sudoku.puzzle.magic);
                solver.initSubsets();
